...
def get_params():
    params = {
        'weight': 'yolov8n.pt',
        'cfg': '/root/autodl-tmp/ultralytics-main/ultralytics-main/ultralytics/cfg/models/v8/underwater.yaml',
        'device': 'cuda:0',     #运行的设备,cpu:cpu,gpu:cuda:0
        'method': 'GradCAM',  # GradCAMPlusPlus, GradCAM, XGradCAM
        'layer': 'model.model[9]',  #可自己测试加在5-9哪一层
        'backward_type': 'all',  # class, box, all
        'conf_threshold': 0.6,  # 置信度阈值,默认0.6,可调整
        'ratio': 0.02  # 0.02-0.1
    }
    return params
...
if __name__ == '__main__':
    model = yolov8_heatmap(**get_params())    #初始化,运行后会输出网络结构和一行代码(分子不等于分母则cfg文件和模型权重不匹配)
    model(r'/root/autodl-tmp/ultralytics-main/ultralytics-main/datasets/underwaterdata/images/val/000008.jpg', 'result')